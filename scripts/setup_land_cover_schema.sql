...
-- Create PARCEL_LAND_COVER main data table
CREATE OR REPLACE TABLE PARCEL_LAND_COVER (
...
-- Create PARCEL_LAND_COVER_SUMMARY for aggregated data per parcel
CREATE OR REPLACE TABLE PARCEL_LAND_COVER_SUMMARY (
...
CREATE INDEX IF NOT EXISTS IDX_PARCEL_LAND_COVER_PARCEL_ID ON PARCEL_LAND_COVER(PARCEL_ID);
CREATE INDEX IF NOT EXISTS IDX_PARCEL_LAND_COVER_NLCD_CODE ON PARCEL_LAND_COVER(NLCD_CODE);
CREATE INDEX IF NOT EXISTS IDX_PARCEL_LAND_COVER_YEAR ON PARCEL_LAND_COVER(NLCD_YEAR);
CREATE INDEX IF NOT EXISTS IDX_PARCEL_LAND_COVER_DOMINANT ON PARCEL_LAND_COVER(IS_DOMINANT_CLASS);
CREATE INDEX IF NOT EXISTS IDX_PARCEL_LAND_COVER_CREATED ON PARCEL_LAND_COVER(CREATED_AT);

CREATE INDEX IF NOT EXISTS IDX_SUMMARY_PARCEL_ID ON PARCEL_LAND_COVER_SUMMARY(PARCEL_ID);
CREATE INDEX IF NOT EXISTS IDX_SUMMARY_DOMINANT_CODE ON PARCEL_LAND_COVER_SUMMARY(DOMINANT_NLCD_CODE);
CREATE INDEX IF NOT EXISTS IDX_SUMMARY_AGRICULTURAL ON PARCEL_LAND_COVER_SUMMARY(IS_AGRICULTURAL);
CREATE INDEX IF NOT EXISTS IDX_SUMMARY_PROCESSED ON PARCEL_LAND_COVER_SUMMARY(LAST_PROCESSED_AT);
...
SELECT 
    ...
    s.NLCD_YEAR,
    s.LAST_PROCESSED_AT
FROM PARCEL_LAND_COVER_SUMMARY s
WHERE s.IS_AGRICULTURAL = TRUE;

CREATE OR REPLACE VIEW VW_PARCEL_LAND_COVER_DETAIL AS
SELECT 
    plc.PARCEL_ID,
    plc.NLCD_YEAR,
    plc.NLCD_CODE,
...
BEGIN
    ...
    DELETE FROM PARCEL_LAND_COVER_SUMMARY WHERE PARCEL_ID = PARCEL_ID_PARAM;
    
    -- Insert new summary
    INSERT INTO PARCEL_LAND_COVER_SUMMARY (
...
CREATE OR REPLACE FUNCTION FN_GET_PARCEL_LAND_COVER_JSON(PARCEL_ID_PARAM VARCHAR(100))
RETURNS VARIANT
LANGUAGE SQL
AS
$$
    SELECT OBJECT_CONSTRUCT(
        ...
        'summary', (
            ...
            FROM PARCEL_LAND_COVER_SUMMARY 
        ...
        'classes', (
            ...
            FROM VW_PARCEL_LAND_COVER_DETAIL
            WHERE PARCEL_ID = PARCEL_ID_PARAM
