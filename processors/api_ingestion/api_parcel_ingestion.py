...
from shared.clients.regrid import RegridClient
from shared.utils.snowflake_jwt_connector import SnowflakeJWTConnector
from shared.utils.dynamodb_cache import DynamoDBCacheManager
        ...
        if topic_arn:
            sns_client.publish(
                ...
                MessageAttributes={
                    'ingestion_type': {
                        'DataType': 'String',
                        'StringValue': detail.get('ingestion_type', 'api')
                    },
                    'query_type': {
                        'DataType': 'String', 
                        'StringValue': detail.get('query_type', 'unknown')
                    }
                }
            )
        else:
